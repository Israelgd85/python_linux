...
def find_links(html):
    ...
    #We're using the perser just to
...
class Spider:
    ...
    process_page() retrieves each page and finds de links.
    """
    ...
    # Checks whether thr link starts with the base URL
        return link.startswith(self.include)

    def process_page(self, url):
    # Retrieve page and finds links in it
        html = get_page(url, self.log)
        for link in find_links(html):
            #Handle relative links
            link  = urlparse.urljoin(url, link)
            self.log("Cheking: " + link)
...
if __name__ == '__main__':
    # This code runs when script is started fom command line
    startURL = sys.argv[1]
